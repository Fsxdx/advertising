...
#         """
#         result = cls.fetch_one(User.sql_provider.get('get_user_by_id.sql', uer_id=user_id))
...
#             Optional[User]: A `User` instance if found, or None otherwise.
#         """
#         result = cls.fetch_one(User.sql_provider.get('get_user_by_email.sql', email=email))
#         return User(*result) if result else None
#
#     @classmethod
...
class SessionManager:
    ...
    def add_user_data(cls, user_id: str, role: str) -> None:
        """Processes login information and sets session data if successful.

        Args:
            user_id (str): Email of the user.
            role (str): Password of the user.
